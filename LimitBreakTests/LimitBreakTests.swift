...
struct ProgressionEngineTests {
    ...
    /// A load too heavy to hold the rep range (you fell below the bottom) is
    /// eased back a notch — the engine doesn't demand reps the weight won't allow.
    @Test func backsOffWhenBelowTheRange() throws {
        let (container, context, bench) = try makeStore()
        // buildMuscle volume band is 10–12; 3×6 @135 sits under the floor.
        logSession(context, exercise: bench, weight: 135, reps: 6, sets: 3, daysAgo: 3)

        let target = ProgressionEngine.nextTarget(
            for: bench, goal: .buildMuscle, emphasis: .volume
        )
        #expect(target?.targetWeightPounds == 130) // eased one 5 lb increment
        #expect(target?.targetReps == 10)          // rebuild from the bottom
        withExtendedLifetime(container) {}
    }

    /// Three sessions stuck at one load with no rep PR → a lighter week, not
    /// another demand to beat a number already missed twice.
    @Test func deloadsAfterAThreeSessionStall() throws {
        let (container, context, bench) = try makeStore()
        // buildMuscle volume band 10–12; 3×11 @135 across three sessions, no gain.
        logSession(context, exercise: bench, weight: 135, reps: 11, sets: 3, daysAgo: 9)
        logSession(context, exercise: bench, weight: 135, reps: 11, sets: 3, daysAgo: 6)
        logSession(context, exercise: bench, weight: 135, reps: 11, sets: 3, daysAgo: 3)

        let target = ProgressionEngine.nextTarget(
            for: bench, goal: .buildMuscle, emphasis: .volume
        )
        #expect(target?.targetWeightPounds == 120) // 135 × 0.9 = 121.5 → snapped to 120
        #expect(target?.targetReps == 11)          // mid of the 10–12 band
        withExtendedLifetime(container) {}
    }
}
